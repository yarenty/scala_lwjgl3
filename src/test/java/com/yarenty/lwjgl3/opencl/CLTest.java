package com.yarenty.lwjgl3.opencl;
...
public class CLTest {
	...
	public  void createCL() {
		try {
			...
			assumeNoException(t);
			//throw new Exception("Skipped because OpenCL initialization failed [" + t.getMessage() + "]");
		} finally {
			CL.destroy();
		}
	}
	...
	private static void contextTest(Filter<CLPlatform> platformFilter, ContextTest test) {
		try {
			...
			if ( platforms.isEmpty() ) {
				...
				throw new RuntimeException("Skipped because the platform filter did not match with any cl_platform.");
			} else {
				for ( CLPlatform platform : platforms ) {
